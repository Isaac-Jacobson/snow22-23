...
def save(img, outPath, num):
    path = outPath + img + "Cropped" + tostring(num)
    print("Image saved to: " + path)
    cv.imwrite(path, Image.open(img))

#Crops each individual snowflake from an image and saves 
#to the output path
#Lots of hardcoded thresholds!!!!
#*****Pulled from an old clab notebook*****
def cropIndividualFlakes(img, out):
    stepSize = 400
    stepSize2 = 20
    threshold1 = 40 
    threshold2 = 150
